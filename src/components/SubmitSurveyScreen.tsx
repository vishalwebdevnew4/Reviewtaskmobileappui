...
import { surveyQueries, userTaskQueries, walletQueries } from '../db/queries';

interface SubmitSurveyScreenProps {
  task: any;
  onNavigate: (screen: string, data?: any) => void;
}

export function SubmitSurveyScreen({ task, onNavigate }: SubmitSurveyScreenProps) {
  const { user } = useAuth();
  ...
  // Parse custom survey fields from task
  const surveyFields = task?.survey_fields ? (() => {
    try {
  ...
  const handleSubmit = () => {
    ...
    for (const field of surveyFields) {
      if (field.required) {
        ...
        if (!response || (typeof response === 'string' && !response.trim())) {
          setError(`Please answer: ${field.label || 'Question ' + field.id}`);
          return;
        }
      }
    }

    // If no custom fields, require default fields
    if (surveyFields.length === 0) {
      ...
      if (surveyText.length < 50) {
        setError('Survey response must be at least 50 characters');
        return;
      }
    ...
    try {
      ...
      const customResponsesJson = Object.keys(customResponses).length > 0 ? JSON.stringify(customResponses) : null;
      surveyQueries.createSurvey({
        taskId: parseInt(task.id),
        userId: user.id,
        rating: rating || undefined,
        surveyText: surveyText || undefined,
        images: uploadedFiles.length > 0 ? JSON.stringify(uploadedFiles) : undefined,
        customResponses: customResponsesJson
      });
  ...
  return (
    <div className="h-full bg-[#F6F6F9] overflow-y-auto pb-24">
      ...
      <div className="sticky top-0 bg-white px-6 py-4 flex items-center gap-4 z-10 shadow-sm">
        ...
        <h2 className="text-[#111111]">Submit Survey</h2>
      </div>
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        {surveyFields.length > 0 ? (
          <div className="space-y-6">
            {surveyFields.map((field, index) => (
              <div key={field.id} className="bg-white rounded-[20px] p-6">
                ...
                {field.type === 'text' && (
                  <input
                    ...
                    className="w-full bg-[#F6F6F9] rounded-[16px] px-4 py-3 outline-none text-[#111111]"
                  />
                )}
                ...
                {field.type === 'radio' && field.options && (
                  <div className="space-y-2">
                    {field.options.map((option, optIdx) => (
                      <label key={optIdx} className="flex items-center gap-3 p-3 bg-[#F6F6F9] rounded-[12px] cursor-pointer">
                        <input
                          type="radio"
                          name={`field-${field.id}`}
                          value={option}
                ...
                {field.type === 'checkbox' && (
                  <div className="flex items-center gap-3 p-3 bg-[#F6F6F9] rounded-[12px]">
                    <input
                      type="checkbox"
                      checked={customResponses[field.id] === true || customResponses[field.id] === 'true'}
                      onChange={(e) => setCustomResponses({...customResponses, [field.id]: e.target.checked})}
                      className="w-4 h-4 text-[#6B4BFF] rounded"
                    />
                    <span className="text-[#111111]">I agree</span>
                  </div>
                )}
              </div>
            ))}
          ...
          <>
            ...
            {/* Survey Text */}
            <div className="bg-white rounded-[20px] p-6">
              <h3 className="text-[#111111] mb-4">Write your survey response</h3>
              <textarea
                value={surveyText}
              ...
              <p className="text-sm text-[#666666] mt-2">
                {surveyText.length} / 50 minimum characters
              </p>
            </div>
          </>
        )}

        {/* Upload Photos (Optional if custom fields exist) */}
        {surveyFields.length === 0 && (
          <div className="bg-white rounded-[20px] p-6">
        ...
        <div className="bg-[#6B4BFF]/10 rounded-[20px] p-6">
          <h3 className="text-[#6B4BFF] mb-3">Survey Guidelines</h3>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white px-6 py-4 shadow-lg">
        <div className="max-w-[414px] mx-auto">
          <Button 
            ...
            disabled={loading || (surveyFields.length === 0 && (rating === 0 || surveyText.length < 50 || uploadedFiles.length < 2))}
          >
